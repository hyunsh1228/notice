...
public class ExceptionController {
	...
	public ModelAndView dataAccess(DataAccessException dae) {
		...
		mView.addObject("exception", dae); // ${exception.getMessage()} = ${exception.message} 는 아래의 뷰페이지에서 사용 가능하다. 
		// view page 설정
		mView.setViewName("error/data_access"); //WEB-INF/views/error/data_access.jsp
		return mView;
	}
}
