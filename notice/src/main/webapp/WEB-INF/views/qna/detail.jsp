...
<div class="container">
	...
	<table class="table table-bordered table-condensed">
		...
		<tr>
			<th>글번호</th><!-- num 인자를 전달 했을 때 글 하나의 정보를 리턴하는 메소드를 Dao에 만들어야 한다. -->
			<td>${dto.num }</td>
		</tr>
		<tr>
			<th>작성자</th>
	...
	--%>
	<c:if test="${dto.writer eq id }"> <!-- 글 작성자와 session에 있는 아이디가 같을 때! -->
	...
	<div class="comments">
		...
		<div class="comment_form">
			...
			<!-- 이 form에는 comment_group 번호가 준비가 안되있으므로 이 폼을 제출하면 Comment_group 번호는 null 값이들어간다. -->
		</div>
	</div>
</div>
<!-- script 영역 -->
<script>
	...
	$(".comment-update-form").on("submit", function(){
		...
		// num=댓글번호&content=댓글내용 (선택자.serialize()를 작성하면 전달됨)
		var data=$(this).serialize(); 
		//이벤트가 일어난 폼을 선택해서 변수에 담아 놓는다.
		var $this=$(this);
		//페이지 전환 없이 요청만 보내기 위해 ajax 사용한다.
		$.ajax({
			url:url,
			method:"post",
			data:data,
		...
		//폼 제출 막기 (submit 버튼을 누르면 폼 제출이 되는데 폼 제출이 되면 페이지 전환이 되므로 막는 것이다.)
		return false;
	});
	...
	function deleteComment(num){
		...
		if(isDelete){
			...
			$.ajax({
				url:"comment_delete.do", // "/cafe/comment_delete.do" 요청 [절대경로 사용해도 됨]
				method:"post",
				data:{"num":num}, // num 이라는 파라미터명으로 삭제할 댓글의 번호 전송 [삭제할 parameter 가져간다]
				success:function(responseData){
					if(responseData.isSuccess){
						var sel="#comment"+num; //아이디가 = comment+전달받은 num 
						$(sel).text("삭제된 댓글 입니다."); //예를들어 num=1이면 comment1을 선택해서 출력한다.
					}
				}
			});
		}
	...
	//폼에 click 이벤트가 일어 났을때 실행할 함수 등록 [focus로 하면 계속 focus가 가서 안된다.]
	...
	$(".comment .reply_link").click(function(){
		...
		.slideToggle(200); //클릭한 그 요소에 부모요소의 부모요소의 부모요소에서  클래스명이 comment-insert-form 인 것을 찾고 0.2초동안 슬라이드로 열린다.
